...
export const IntrinsicValue: React.FC<IntrinsicValueProps> = ({ ticker, displayDetails, withTitle }) => {
    ...
    const items = companyValues?.areas?.map( (_, i) => ({ label: t(`ticker.value.levels.${i}`), command: () => setLevel(i) }));

    return (
        <div>
            {!!withTitle && (<h2 className="bg-primary p-2 flex">
                <div>
            ...
            {!!items && 
                ...
                <div className='ml-auto align-items-center flex mr-3'>
                    ...
                    {!!displayDetails && <i className='ml-3 pi pi-table cursor-pointer hover:text-primary' onClick={() => setDetailsVisible(!detailsVisible)}></i>}
                </div>
            </div>
