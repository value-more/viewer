...
export const CompanyProfile: React.FC<CompanyProfileProps> = ({ cik, edit }) => {
    ...
    const save = (value: string) => {
        ...
        timeout = setTimeout(() => {
            api(`invData/companies/${cik}/profiles`, { method: 'POST', body: JSON.stringify({ text: value, language }) })
            .then( () => {
                toasts.showToast({ severity: 'info', summary: 'Company profile updated' });
            });
        }, 750);
    ...
    return (<>
        {edit ? (
            <h3 className="bg-primary p-2 flex align-items-center">
                <div><i className='pi pi-info mr-2' />{t('ticker.profile')}</div>
